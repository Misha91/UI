""" plotting.py
AE3M33UI - Artificial Intelligence course, FEE CTU in Prague
...
def compute_learning_curve(model, tr_sizes, Xtr, ytr, Xtst, ytst):
    ...
    for k in range(len(tr_sizes)):
        y_tr = ytr[:tr_sizes[k]]
        X_tr = Xtr[:tr_sizes[k], :]

        model.fit(X_tr, y_tr)

        tr_errors[k] = 1 - metrics.accuracy_score(y_tr, model.predict(X_tr))
        tst_errors[k] = 1 - metrics.accuracy_score(ytst[:tr_sizes[k]], model.predict(Xtst[:tr_sizes[k], :]))
...
def compute_validation_curve(model, param_name, param_range, Xtr, ytr, Xtst, ytst):
    ...
    for k, p in enumerate(param_range):
        para = {param_name:p}
        model.set_params(**para)
        model.fit(Xtr, ytr)

        tr_errors[k] = 1 - metrics.accuracy_score(ytr, model.predict(Xtr))
        tst_errors[k] = 1 - metrics.accuracy_score(ytst, model.predict(Xtst))
    # <YOUR CODE HERE>
